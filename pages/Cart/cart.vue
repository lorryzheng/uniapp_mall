<template>
	<view>
	    ...
	    <view class='empty-tips' v-if='cartList.length == 0'>暂无商品</view>
	    <view class='cart-list-box' v-for='(item,index) in cartList' :key='index'>
	        <view class='cart-shop'>
	            <check-box class='shop-select' checked='item.selected' @click='shopClick'></check-box>
	        ...
	        <view class='cart-item' v-for='(item,index) in skuList' :key='index'>
	            <!-- <movable-area class='cart-item-movable'>
	                <mo -->vable-view class='cart-item-movable-view' damping="100" out-of-bounds="false" direction="horizontal" x="{{x[index]}}" animation="false" bindtouchend="handleTouchend" bindchange="handleMovableChange" bindtouchstart="handleTouchestart" data-index='{{index}}'>
	                    <view class="cart-main-box">
	                        ...
	                        <view class='goods-info-box' @click="goGoodsDetail">
	                            ...
	                            <view class='goods-format'>
	                                <text v-if='item.color'>{{item.color}}，</text>
	                                <text v-if='item.lining'>{{item.lining}}，</text>
	                                <text v-if='item.size'>{{item.size}}，</text>
	                                <text>{{item.num}}件</text>
	                            </view>
	                            <!-- <image class='goods-label' src='https://mozi.bailingyoupin.com/static/images/wx_sm/home/tags_icon.png' mode='widthFix' v-if='{{item.type == 2}}'></image> -->
	                    ...
	                    <view class="cart-delete">移除</view>
	                <!-- </movable-view> -->
	            <!-- </movable-area> -->
	        </view>
	    </view>
	    ...
	    <view class='balance-box'>
	        <check-box class='all-select' checked='allSelect' @click='allClick'></check-box>
	        ...
	        <view class='banlance-btn' v-if='editStatus == 0' @click='goOrderConfirm'>结算</view>
	        <view class='delete-btn' v-else @click='goDelete'>移除</view>
	    </view>
	</view>
</template>
